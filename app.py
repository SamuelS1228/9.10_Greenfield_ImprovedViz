...
def sidebar(scn):
    ...
    with st.sidebar:
        ...
        st.markdown("**Brand-specific Canada settings**")
        scn["brand_can_thresh"] = scn.get("brand_can_thresh", {})
        scn["brand_can_override"] = scn.get("brand_can_override", {})
        brands = scn.get("brands", [])
        for b in brands:
            cc1, cc2 = st.columns([2,2])
            val = float(scn["brand_can_thresh"].get(b, scn.get("can_lon", -105.0)))
            scn["brand_can_thresh"][b] = cc1.number_input(f"{b} — Longitude threshold", value=val, format="%.3f", key=f"can_{b}_lon_{name}")
            scn["brand_can_override"][b] = cc2.checkbox(f"{b} — Brand constraints override Canada routing", value=bool(scn["brand_can_override"].get(b, False)), key=f"can_{b}_ovr_{name}")
        ...
        st.divider()
        st.checkbox("Enable brand-level service level targets", key=f"en_brandsl_{name}", value=scn.get("en_brandsl", False))
        scn["en_brandsl"] = st.session_state.get(f"en_brandsl_{name}")
        brand_sl_targets = {}
        if scn.get("en_brandsl", False):
            brands = scn.get("brands", [])
            for b in brands:
                st.markdown(f"**Brand: {b}**")
                c1,c2 = st.columns(2)
                v1 = c1.number_input(f"{b} — 0–350 mi %", min_value=0.0, max_value=100.0, step=1.0,
                                     value=float(scn.get(f"bsl_{b}_by7", 0.0)), key=f"bsl_{b}_by7_{name}")
                v2 = c2.number_input(f"{b} — 351–500 mi %", min_value=0.0, max_value=100.0, step=1.0,
                                     value=float(scn.get(f"bsl_{b}_by10", 0.0)), key=f"bsl_{b}_by10_{name}")
                c3,c4 = st.columns(2)
                v3 = c3.number_input(f"{b} — 501–700 mi %", min_value=0.0, max_value=100.0, step=1.0,
                                     value=float(scn.get(f"bsl_{b}_eod", 0.0)), key=f"bsl_{b}_eod_{name}")
                v4 = c4.number_input(f"{b} — 701+ mi %", min_value=0.0, max_value=100.0, step=1.0,
                                     value=float(scn.get(f"bsl_{b}_2d", 0.0)), key=f"bsl_{b}_2d_{name}")
                scn[f"bsl_{b}_by7"], scn[f"bsl_{b}_by10"], scn[f"bsl_{b}_eod"], scn[f"bsl_{b}_2d"] = v1, v2, v3, v4
                brand_sl_targets[b] = {"by7": v1/100.0, "by10": v2/100.0, "eod": v3/100.0, "2day": v4/100.0}
        scn["brand_sl_targets"] = brand_sl_targets
        ...
        if st.session_state.get("run_target")==name:
            with st.spinner("Optimizing…"):
                ...
                res=optimize(
                    ...
                    brand_service_level_targets=scn.get("brand_sl_targets", {}),
                    enforce_brand_service_levels=bool(scn.get("en_brandsl", False)),
                    brand_canada_thresholds=scn.get("brand_can_thresh", {}),
                    brand_override_canada=scn.get("brand_can_override", {}),
                    enforce_service_levels=bool(scn.get("sl_enforce", False)),
            ...
            # Brand visualization selector
            brands_present = sorted([str(x) for x in res["assigned"].get("Brand", pd.Series([])).astype(str).dropna().unique().tolist()])
            sel_brand = None
            if brands_present:
                sel_brand = st.selectbox("Select brand to visualize", options=["(All)"] + brands_present, index=0)
            vis_df = res["assigned"]
            if sel_brand and sel_brand != "(All)":
                vis_df = vis_df[vis_df["Brand"].astype(str) == sel_brand]
            plot_network(vis_df,res["centers"])
            ...
            # Brand-level service levels
            bsl = res.get("brand_service_levels", {})
            b_tgts = res.get("brand_sl_targets", {})
            if bsl:
                st.subheader("Brand Service Levels")
                rows = []
                for bname, vals in sorted(bsl.items()):
                    tgt_row = b_tgts.get(str(bname), {})
                    rows.append({
                        "Brand": bname,
                        "≤350 (7AM)": _pct(vals.get("by7",0.0)),
                        "351–500 (10AM)": _pct(vals.get("by10",0.0)),
                        "501–700 (EOD)": _pct(vals.get("eod",0.0)),
                        ">701 (2d+)": _pct(vals.get("2day",0.0)),
                        "Targets": ", ".join([f"{k}:{_pct(tgt_row.get(k,0))}" for k in ("by7","by10","eod","2day") if k in tgt_row])
                    })
